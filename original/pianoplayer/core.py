...
def annotate_fingers_xml(sf, hand, args, is_right=True):
    ...
    for el in p0.flatten().getElementsByClass("GeneralNote"):
        # --- [START] NEW SYNCHRONIZATION LOGIC ---
        
        # 1. Skip rests, as they do not get a fingering.
        if el.isRest:
            continue

        # 2. Skip grace notes, which are identified by their zero duration.
        if el.duration.quarterLength == 0:
            continue

        # 3. Skip notes that are the continuation or end of a tie.
        #    Only the note that starts the tie should get a fingering.
        if el.tie is not None and el.tie.type in ['stop', 'continue']:
            continue
            
        # --- [END] NEW SYNCHRONIZATION LOGIC ---

        # Final safety check: If we have run out of calculated fingerings,
        # print a warning and stop annotating this hand to prevent a crash.
        if idx >= len(hand.noteseq):
            print(f"[WARNING] Note sequence desynchronized for {'Right' if is_right else 'Left'} hand. "
                  f"Stopping annotation for this hand to prevent a crash.")
            break

        # --- Original annotation logic ---
        if el.isNote:
            n = hand.noteseq[idx]
            if hand.lyrics:
            ...
            # For chords, iterate through each note of the chord
            for j, cn in enumerate(el.pitches):
                # We need another safety check here for the inner loop
                if idx >= len(hand.noteseq):
                    print(f"[WARNING] Ran out of fingerings while processing a chord for {'Right' if is_right else 'Left'} hand.")
            ...
            # If the inner loop was broken, we should also break the outer one
            if idx >= len(hand.noteseq) and el.isChord:
                break
...
def annotate(args):
    ...
    if '.msc' in args.filename:
        ...
        # --- [START] NEW SINGLE-STAFF LOGIC ---
        if len(sc.parts) == 1:
            print("[INFO] Single-staff score detected. Determining hand based on clef...")
            part = sc.parts[0]
            # Find the first clef in the part to determine the hand
            clef = part.getElementsByClass('Clef').first()
            
            # Default to Right Hand if no clef is found
            is_left_hand = False
            if clef:
                # 'F' clef (Bass clef) implies Left Hand
                if clef.sign == 'F':
                    is_left_hand = True
            
            if is_left_hand:
                print("[INFO] Bass clef found. Assigning to Left Hand.")
                # By setting left_only, we ensure the right hand processing block is skipped
                args.left_only = True
                # We tell the left hand processor to read from the first (and only) part at index 0
                args.lbeam = 0
            else:
                print("[INFO] Treble or other clef found. Assigning to Right Hand.")
                # By setting right_only, we ensure the left hand processing block is skipped
                args.right_only = True
                # We tell the right hand processor to read from the first (and only) part at index 0
                args.rbeam = 0
        # --- [END] NEW SINGLE-STAFF LOGIC ---

        if not args.left_only:
            rh_noteseq = reader(sc, beam=args.rbeam)
            print(f"[INFO] Right Hand (beam {args.rbeam}): Found {len(rh_noteseq)} notes to be fingered.")
        if not args.right_only:
            lh_noteseq = reader(sc, beam=args.lbeam)
            print(f"[INFO] Left Hand (beam {args.lbeam}): Found {len(lh_noteseq)} notes to be fingered.")
